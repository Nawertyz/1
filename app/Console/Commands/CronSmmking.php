...
class CronSmmking extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cron:smmking';
     ...
     * @var string
     */
    protected $description = 'Cron service smmking';
    ...
    public function handle()
    {
        ...
        $this->info('Cron smmking: ' . date('d-m-Y H:i:s'));
        $data = Orders::where('actual_service', 'smmking')
                ...
                if ($status == 'Canceled') {
                 ...
                 if ($status == 'Partial') {
                    $order_history[] = [
                        'time' => date('H:i d/m/Y'),
                        'status' => 'danger',
                        'title' => "Đơn hàng đã chạy một phần",
                    ];
                    $status="Cancelled";
                    $item->history = json_encode($order_history);
                }
        ...
        $this->info('Cron smmking: ' . $count . ' orders');
