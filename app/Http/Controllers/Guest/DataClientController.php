...
class DataClientController extends Controller
{
    ...
    public function Card(Request $request)
    {
        $valid = Validator::make($request->all(), [
            ...
            'telco.required' => 'Vui lòng chọn loại thẻ cần nạp',
            'serial.required' => 'Se ri không được bỏ trống.',
            'code.required' => 'Mã thẻ không được bỏ trống',
        ]);
        ...
        if ($valid->fails()) {
            ...
            $data = trumcard1s(DataSite('partner_id'), $telco, $request->code, $request->serial, $request->amount, $request_id, $sign);
            if (isset($data)) {
                if ($data['status'] == 99) {
                    HistoryCard::create([
    ...
    public function UpdateProfile($type, Request $request)
    {
        ...
        if ($type == 'change-password') {
            $valid = Validator::make($request->all(), [
                ...
                'confirm_password.required' => 'Mật khẩu xác nhận lại không được bỏ trống',
            ]);
        ...
        if ($type == 'update-telegram') {
            $user = Auth::user();
            $user->telegram_chat_id = $request->telegram_id;
            $user->save();
            return resApi('success', "Cập nhật thành công");
        }
    }
    ...
    public function ListAction($action, Request $request)
    {
        ...
        if ($action == 'history-user') {
            ...
            $data = $data->map(function ($item) {
                $item->new_dataa = change($item->old_data,$item->new_data,$item->data);
                return $item;
            });
        ...
        if ($action == 'history-tientrinh') {
            if (!empty($search)) {
                $data = Orders::where('domain', getDomain())->where('username', Auth::user()->username)->where(function ($query) use ($search) {
                    $query->where('username', 'like', "%$search%")
                ...
                $data = Orders::where('domain', getDomain())->where('username', Auth::user()->username)->orderBy('id', $dir)->offset($start)->limit($length)->get();
                $total = Orders::where('domain', getDomain())->where('username', Auth::user()->username)->count();
            }
            $data = $data->map(function ($item) {
        ...
        if ($action == 'history-support') {
            if (!empty($search)) {
                $data = Support::where('domain', getDomain())->where('username', Auth::user()->username)->where(function ($query) use ($search) {
                ...
                $data = Support::where('domain', getDomain())->where('username', Auth::user()->username)->orderBy('id', $dir)->offset($start)->limit($length)->get();
                $total = Support::where('domain', getDomain())->where('username', Auth::user()->username)->count();
            }
            $data = $data->map(function ($item) {
    ...
    public function OrderAction($social, $action, Request $request)
    {
        ...
        $service = Service::where('domain', env('PARENT_SITE'))->where('slug', $action)->where('service_social', $social)->first();
            ...
            $data = $data->map(function ($item) {
                $item->status_order = statusOrder($item->status);
                $item->phantram = round(($item->buff / $item->quantity) * 100, 2);
                return $item;
            });
