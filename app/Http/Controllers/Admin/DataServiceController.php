    ...
    public function tainguyenNewTainguyen(Request $request)
    {
        ...
        if ($valid->fails()) {
            return resApi('error', $valid->errors()->first());
                ...
                $tainguyen = Category::where('domain', getDomain())->where('id', $request->chuyenmuc)->first();
                if ($tainguyen) {
                    ...
                    return resApi('error', 'Không tìm thấy chuyên mục');
                }
    ...
    public function serverEdit($id, Request $request)
    {
        ...
        if ($valid->fails()) {
            ...
            if (getDomain() == env('PARENT_SITE')) {
                ...
                $sitecon = SiteCon::where('domain', getDomain())->get();
            $ser= ServerService::where('domain', getDomain())->where('id', $id)->first();
             foreach ($sitecon as $upsitecon){
                 $sitedata = SiteData::where('domain', $upsitecon['domain_name'])->first();
                 if(isset($sitedata)){
               ServerService::where('domain', $upsitecon['domain_name'])->where('social_id', $ser->social_id)->where('socialll', $ser->socialll)->where('service_id', $ser->service_id)->where('server', $ser->server)->update([
                      'server' => $request->server_service,
                'price' => round($request->price + $request->price * ($sitedata['price'] / 100), 2),
                'price_collaborator' => round($request->price_collaborator + $request->price_collaborator * ($sitedata['price_collaborator'] / 100), 2),
                'price_agency' => round($request->price_agency + $request->price_agency * ($sitedata['price_agency'] / 100), 2),
                'price_distributor' => round($request->price_distributor + $request->price_distributor * ($sitedata['price_distributor'] / 100), 2),
                  'min' => $request->min,
                    'max' => $request->max,
                    'title' => $request->title,
                    'description' => $request->description,
...
public function orderEdit($id, Request $request)
     ...
     public function orderRefund(Request $request)
    {
        ...
        if ($valid->fails()) {
            ...
            $order = Orders::where('domain', getDomain())->where('id', $request->id)->update([
                'status' => 'Hoan',
            ]);

            if ($order) {
                $order11 = Orders::where('domain', getDomain())->where('id', $request->id)->first();
                $user = User::where('domain', getDomain())->where('username', $order11->username)->first();
                if ($user) {
                    if ($order11->buff != ''){
                          $tien = $order11->total_payment - ($order11->price * $order11->buff);
                    }
                  else{
                        $tien = $order11->total_payment;
                  }
                       ...
                       DataHistory::create([
                                                                    ...
                                                                    'new_data' => $user->balance + $tien,
                                                                    'ip' => $request->ip(),
                                                                    'description' => "Hoàn tiền đơn hàng #".$request->id,
                                                                    'data_json' => '',
                                                                    'domain' => getDomain(),
                                                                ]);
                    $user->balance = $user->balance + $tien;
                    $user->total_deduct = $user->total_deduct - $tien;
                    $user->save();
                }
                return resApi('success', 'Hoàn tiền thành công đơn #'.$request->id);
            } else {
                return resApi('error', 'Không tìm thấy đơn hàng');
            }
        }
